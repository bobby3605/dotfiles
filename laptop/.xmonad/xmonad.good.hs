...
--
myWorkspaces = withScreens 2["1: term","2: web","3: code","4: media"] ++ map show [5..9]
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  , ((0, xF86XK_AudioLowerVolume),
     spawn "amixer -c 1 -q set Master 1%-")
  ...
  -- Audio previous.
  , ((0, xF86XK_AudioPrev),
     spawn "playerctl --all-players previous")

  -- Play/pause.
  , ((0, xF86XK_AudioPlay),
     spawn "playerctl --all-players play-pause")

  -- Audio next.
  , ((0, xF86XK_AudioNext),
     spawn "playerctl --all-players next")
...
main = do
  n <- countScreens
  xmprocs <- mapM (\i -> spawnPipe $ "xmobar -x"++ show i ++ " ~/.xmonad/xmobarrc.hs") [0..n-1]
  xmonad $ docks
         ...
         logHook = dynamicLogWithPP xmobarPP {
                ...
                , ppSep = "   "
                , ppOutput = hPutStrLn $ head xmprocs
         } >> updatePointer (0.75, 0.75) (0.75, 0.75) >> dynamicLogWithPP xmobarPP{
                  ppCurrent = xmobarColor xmobarCurrentWorkspaceColor "" . wrap "[" "]"
                , ppTitle = xmobarColor xmobarTitleColor "" . shorten 50
                , ppSep = "   "
                , ppOutput = hPutStrLn $ xmprocs !! 1
         }
         }
------------------------------------------------------------------------
